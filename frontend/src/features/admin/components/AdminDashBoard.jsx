...
export const AdminDashBoard = () => {
  ...
  // Reset page khi thay đổi kết quả tìm được
  useEffect(() => {
    setPage(1)
  }, [totalResults])

  // Gửi request mỗi khi filters/sort/page thay đổi
  useEffect(() => {
    const finalFilters = { ...filters }
    finalFilters['pagination'] = { page: page, limit: ITEMS_PER_PAGE }
    finalFilters['sort'] = sort || null
        ...
        <Stack mb={'5rem'} sx={{ scrollBehavior: "smooth", overflowY: "scroll" }}>
          ...
          {/* Sport Categories */}
            ...
            {[
                ...
                <Typography key={i} sx={{ cursor: "pointer" }} variant='body2'>
                {txt}
                </Typography>
            ))}
            </Stack>
        ...
        <Grid gap={2} container justifyContent={'center'}>
          ...
          {products.map((product) => (
            <Stack key={product._id}>
              ...
              <Stack paddingX={2} flexDirection={'row'} alignSelf={'flex-end'} columnGap={is488 ? 1 : 2}>
        ...
        <Stack alignSelf={is488 ? 'center' : 'flex-end'} mr={is488 ? 0 : 5} rowGap={2} p={is488 ? 1 : 0}>
          ...
          <Typography textAlign={'center'}>
            Showing {(page - 1) * ITEMS_PER_PAGE + 1} to {
              page * ITEMS_PER_PAGE > totalResults ? totalResults : page * ITEMS_PER_PAGE
