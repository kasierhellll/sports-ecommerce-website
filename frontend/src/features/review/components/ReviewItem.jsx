...
export const ReviewItem = ({id,username,userid,comment,rating,createdAt}) => {
  ...
  return (
    <Stack position={'relative'} p={2} rowGap={2} width={'100%'} component={Paper} borderRadius={'.8px'}>
        ...
        {/* người đánh giá, điểm sao, ngày đánh giá */}
...

          {
            isOwnReview && (
              ...
              <Menu anchorEl={anchorEl} open={open} onClose={handleClose}>
                <MenuItem onClick={()=>{setEdit(true);handleClose()}}>Chỉnh sửa</MenuItem>
                <MenuItem onClick={deleteReview}>Xóa</MenuItem>
              </Menu>
            </Stack>
            )
          }

            <Typography alignSelf={"flex-end"} justifySelf={'flex-end'} color={'text.secondary'} fontWeight={400} fontSize={'.9rem'}>
            ...
            edit?(
              <Stack component={'form'} noValidate onSubmit={handleSubmit(handleUpdateReview)} rowGap={2}>
                ...
                <Stack flexDirection={'row'} alignSelf={'flex-end'} columnGap={1} justifyContent={'center'} alignItems={'center'}>
                  <LoadingButton size='small' type='submit' sx={{alignSelf:"flex-end"}} variant='contained'>Cập nhật</LoadingButton>
                  <Button variant='outlined' size='small' onClick={()=>setEdit(false)} color='error'>Hủy</Button>
                </Stack>
              </Stack>
