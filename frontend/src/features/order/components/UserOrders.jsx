    ...
    <Stack justifyContent={'center'} alignItems={'center'}>
        {
            ...
            <Stack width={is1200?"auto":"60rem"} p={is480?2:4} mb={'5rem'}>
                ...
                <Stack flexDirection={'row'} columnGap={2} >
                    ...
                    <Stack rowGap={1} >
                        <Typography variant='h4' fontWeight={500}>Lịch sử đơn hàng</Typography>
                        <Typography sx={{wordWrap:"break-word"}} color={'text.secondary'}>Xem trạng thái các đơn hàng gần đây, quản lý đổi trả, và tìm sản phẩm tương tự.</Typography>
                    </Stack>
                ...
                <Stack mt={5} rowGap={5}>
                                    ...
                                    <Stack flexDirection={'row'} rowGap={'1rem'}  justifyContent={'space-between'} flexWrap={'wrap'}>
                                        <Stack flexDirection={'row'} columnGap={4} rowGap={'1rem'} flexWrap={'wrap'}>
                                            ...
                                            <Stack>
                                                ...
                                                <Typography color={'text.secondary'}>{new Date(order.createdAt).toLocaleDateString('vi-VN')}</Typography>
                                            </Stack>

                                            <Stack>
                                                <Typography>Tổng tiền</Typography>
                                                <Typography>{order.total.toLocaleString('vi-VN')} ₫</Typography>
                                            </Stack>
                                        </Stack>
                                                    ...
                                                    <Stack rowGap={1} width={'100%'}>
                                                        ...
                                                        <Stack flexDirection={'row'} justifyContent={'space-between'}>
                                                            <Stack>
                                                                ...
                                                                <Typography color={'text.secondary'} fontSize={'.9rem'}>Số lượng: {product.quantity}</Typography>
                                                            </Stack>
                                                            <Typography>{product.product.price.toLocaleString('vi-VN')} ₫</Typography>
                                                        </Stack>
                                                        ...
                                                        <Stack mt={2} alignSelf={is480?"flex-start":'flex-end'} flexDirection={'row'} columnGap={2} >
                                                            <Button size='small' component={Link} to={`/product-details/${product.product._id}`} variant='outlined'>Xem sản phẩm</Button>
                                                            {
                                                                cartItems.some((cartItem)=>cartItem.product._id===product.product._id)?
                                                                <Button  size='small' variant='contained' component={Link} to={"/cart"}>Đã có trong giỏ</Button>
                        ...
                        !orders.length && 
                            <Stack mt={is480?'2rem':0} mb={'7rem'} alignSelf={'center'} rowGap={2}>
                                ...
                                <Typography textAlign={'center'} alignSelf={'center'} variant='h6' >Ồ! Có vẻ như bạn chưa mua sắm gần đây</Typography>
